    ...
    def run(self, cache_ops_inp) ->  None:
        ...
        for ins in range(len(cache_ops)):
            if cache_ops[ins][0] == 'R':
                self._cache.read_from_cache(cache_ops[ins][1])
            elif cache_ops[ins][0] == 'W':
                self._cache.write_to_cache(cache_ops[ins][1], cache_ops[ins][2])
            else:
...
def main():
    ...
    parser.add_argument('--cache_assoc', '-ca', default=1, type=int,help='The cache associativity. example: 4', metavar='cache_associativity', dest='cache_assoc')
